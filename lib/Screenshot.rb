...
module Dhalang
  class Screenshot
    ...
    def self.get_from_url_as_png(url)
      ...
      begin
        visit_page_with_puppeteer(url, temporary_screenshot_save_file.path)
        binary_png_content = get_file_content_as_binary_string(temporary_screenshot_save_file)
      ensure
        temporary_screenshot_save_file.close unless temporary_screenshot_save_file.closed?
        temporary_screenshot_save_file.unlink
      end
      return binary_png_content
    end
